<div class="card my-2 bg-light">
    <div class="card-body">
        ...
        @can('update', $publication)
            <a class="float-end btn btn-primary" href="{{ route('publications.edit', $publication->id) }}">Modifier</a>
        @endcan
                ...
                <footer class="card-blockquote">
                    ...
                    @can('delete', $publication)
                        <form action="{{ route('publications.destroy', $publication->id) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <button onclick="return confirm('voulez vous suprimer ca')"
                                class="btn btn-danger float-end">Suprimer</button>
                        </form>
                    @endcan
